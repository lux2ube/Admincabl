...
function ProductImage({
  ...
  return <img className={className} src={catalogImageUrl(image, 640)} srcSet={catalogImageSrcSet(image)} sizes={sizes} alt={product.productName} width={640} height={640} loading={loading} fetchPriority={fetchPriority} decoding="async" onError={() => setImageFailed(true)} data-testid={`img-detail-${product.id}`}/>;
}
...
function BrandCollectionImage({ product, brand }: { product?: StoreProduct; brand: string }) {
  ...
  return <span className="brand-collection-image">
    {failed || !product?.images?.[0]
      ...
      : <img src={catalogImageUrl(product.images[0], 320)} srcSet={catalogImageSrcSet(product.images[0], [160, 320, 480])} sizes="160px" alt="" width={320} height={320} loading="lazy" decoding="async" onError={() => setFailed(true)} />}
  </span>;
}
...
const categoryGuideCopy: Record<string, {
  ...
  chargers: {
    ...
    faqs: [
      { question: 'هل الشاحن الأعلى واتاً يشحن كل الأجهزة بسرعة أكبر؟', answer: 'لا. الجهاز والكابل والبروتوكول والحرارة تحدد القدرة الفعلية.' },
  ...
  cables: {
    ...
    tips: ['حدد USB-C إلى USB-C أو USB-A إلى USB-C أو Lightning حسب أجهزتك.', 'راجع القدرة بالوات إذا كنت تستخدم شحناً سريعاً أو لابتوباً.', 'لا تعتبر كل كابلات USB-C متساوية في نقل البيانات أو الفيديو.', 'اختر طولاً يناسب مكان الاستخدام دون شد أو ثني حاد.', 'استبدل الكابل عند تلف الغلاف أو ارتفاع الحرارة أو انقطاع الاتصال.'],
    faqs: [
      ...
      <div className="container product-detail">
        ...
        <div className="product-detail-grid">
          <div className="detail-gallery">
            ...
            <div className="thumb-row">{product.images.map((image, index) => <button className={`thumb ${imageIndex === index ? 'active' : ''}`} key={image} onClick={() => setImageIndex(index)} data-testid={`button-product-thumb-${index}`}><img src={catalogImageUrl(image, 160)} srcSet={catalogImageSrcSet(image, [120, 160, 240])} sizes="68px" width={160} height={160} loading="lazy" decoding="async" alt=""/></button>)}</div>
          </div>
  ...
  return (
    <>
      ...
      <div className="container checkout-page">
        ...
        <form {...{ toolname: 'start-cabl-checkout', tooldescription: 'Start checkout for the products currently in the CABL cart; the shopper must review and submit the order.' }} className="checkout-layout" onSubmit={submit}>
