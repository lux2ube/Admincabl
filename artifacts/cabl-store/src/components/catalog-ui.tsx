...
export function ProductCard({ product, compact = false, compareSelected = false, onToggleCompare }: { product: StoreProduct; compact?: boolean; compareSelected?: boolean; onToggleCompare?: (productId: string) => void }) {
  ...
  return <article className={`product-card ${compact ? 'compact' : ''} reveal`} data-testid={`card-product-${product.id}`}>
    ...
    <div className="product-copy"><span className="product-brand">{product.brand}</span><Link href={productPath(product)} className="product-title" data-testid={`text-product-name-${product.id}`}>{product.productName}</Link>{product.shortDescription && <p>{product.shortDescription}</p>}{product.productNote && <div className="product-decision-meta"><span className="product-note" title={product.productNote}>{product.productNote}</span></div>}<div className="product-meta"><div><strong data-testid={`text-price-${product.id}`}>{formatPrice(price)}</strong>{discount > 0 && <del>{formatPrice(product.regularPrice)}</del>}</div><button className={`add-button${added ? ' is-success' : ''}`} onClick={handleAddToCart} disabled={product.quantity < 1} aria-live="polite" data-testid={`button-add-cart-${product.id}`}><ShoppingBag size={16}/><span>{added ? 'تمت الإضافة' : product.quantity > 0 ? 'أضف للسلة' : 'نفد'}</span></button></div></div>
  </article>;
}
